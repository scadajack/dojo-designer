...
//	"dojo",
	...
	"dojo/on",
	"dojo/query"
], function ( /* dojo , */ lang, array, aspect, Container, Selector, Source, dnd, Manager, dom, domClass, mouse, NodeList, on, query) {
	...
	lang.extend( Container, {
		...
		getAllNodes: function( ) {
			...
			if(this.dropWidget && 'getChildren' in this.dropWidget) {
				var t = new NodeList();
				array.forEach(this.dropWidget.getChildren(), function(w) {
					t.push(w.domNode);
				});
				return t;
			}
			return query("> .dojoDndItem", this.parent);
		},
	...
	lang.extend( Selector, {
		postscript: function( ) {
			Selector.superclass.postscript.apply( this, arguments );
			...
			this.events.push( aspect.after(this, "_addItemClass", lang.hitch(this, "_onClassChange"), true) );
			this.events.push( aspect.after(this, "_removeItemClass", lang.hitch(this, "_onClassChange"), true) );
		},
		...
		isSelected: function( node ) {
			...
			return domClass.contains( node, "dojoDndItemSelected" ) || domClass.contains( node, "dojoDndItemAnchor" );
		},
		...
		select: function( node ) {
			...
			if( lang.isArray( node ) ) {
				array.forEach( node, this.select, this );
				return;
			}		
		...
		deselect: function( node ) {
			...
			if( lang.isArray( node ) ) {
				array.forEach( node, this.deselect, this );
				return;
			}
		...
		insertNodes: function(addSelected, data, before, anchor){
			...
			this._normalizedCreator = function(item, hint){
				...
				t.handle && domClass.toggle( t.handle, "dojoDndHandle", true );
				t.selector && domClass.toggle( t.selector, "dojoDndSelector", true );
		...
		onOverEvent: function(){
			...
			this._deferredConnects = [
				on(this.node, "click", lang.hitch(this, "onClick")),
				on(this.node, "mousemove", lang.hitch(this, "onMouseMove"))
			];
		},
		...
		onOutEvent: function(){
			...
			if( this._deferredConnects ) {
				for( var i = 0; i < this._deferredConnects.length; i++ ) {
					this._deferredConnects[ i ].remove();
				}
			}
		...
		_onClassChange: function( node, type ) {
			...
			if( type === "Selected" || type === "Anchor" || !node.hasAttribute("aria-selected") ) {
...
//				dijit.setWaiState( node, "selected", selected );
	...
	lang.extend( Source, {
		...
		onDndCancel: function(){
			...
			var m = manager, target = m.target, source = m.source;
			var target = m.target;
		...
		onDropExternal: function(source, nodes, copy){
			...
			if(this.creator){
				...
				if(copy){
					this._normalizedCreator = function(node, hint){
						return Source._copyItem( source.getItem(node.id) );
					};
				}else{
					this._normalizedCreator = function(node, hint){
		...
		onDropInternal: function(nodes, copy){
			...
			if(copy){
				if(this.creator){
					...
					this._normalizedCreator = function(node, hint){
						// support for the new dojo.dnd.Item
						return Source._copyItem( this.getItem( node.id ) );
					};
				}
			}else{
				if(!this.current){
		...
		_isValidMouseOverOut: function( ) {
			...
			var m = manager, s = m.source, n = m.nodes;
	...
	Source._copyItem = function( i ) {
		...
		var h, s;
		if( i.handle ) {
			h = dom.byId( i.handle.id, n );
			h.id = n.id + "_handle";
		}
		if( i.selector ) {
			s = dom.byId( i.selector.id, n );
			s.id = n.id + "_selector";
		}
